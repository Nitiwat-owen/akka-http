...
object MarshallingJSON extends App
  ...
  val rtjvmGaneMap = system.actorOf(Props[GameAreaMap], "rockTheJVMGameAreaMap")
  val playerList = List(
    Player("martin killz u", "Warrior", 70),
    Player("rolandbraveheart007", "Elf", 67),
    Player("deniel_rock03", "Wizard", 30)
  )

  playerList.foreach( {player =>
    rtjvmGaneMap ! AddPlayer(player)
  })
   ...
   * - GET /api/player?nickname=X, does the same as above
   * - GET /api/player/class/{charClass}, returns all the players with the giver character class
    ...
    pathPrefix("api" / "player") {
      get {
        path("class" / Segment) {characterClass =>
          val playersByClassFuture = (rtjvmGaneMap ? GetPlayerByClass(characterClass)).mapTo[List[Player]]
          complete(playersByClassFuture)
        } ~
          (path(Segment) | parameter('nickname.as[String])) {nickname =>
            val playerOptionFuture = (rtjvmGaneMap ? GetPlayer(nickname)).mapTo[Option[Player]]
            complete(playerOptionFuture)
          } ~
          pathEndOrSingleSlash {
            complete((rtjvmGaneMap ? GetAllPlayers).mapTo[List[Player]])
          }
      } ~
        post {
          entity(as[Player]) { player =>
            complete((rtjvmGaneMap ? AddPlayer(player)).map(_ => StatusCodes.OK))
          }
        }~
        delete {
          entity(as[Player]) { player =>
            complete((rtjvmGaneMap ? RemovePlayer(player)).map(_ => StatusCodes.OK))
          }
        }
    }
