  ...
  "A digital library backend" should {
    ...
    "return a book by hitting the query parameter endpoint" in {
      Get("/api/book?id=2") ~> libraryRoute ~> check {
        status shouldBe StatusCodes.OK
        responseAs[Option[Book]] shouldBe Some(Book(2, "JRR Tolkien", "The Load of the Rings"))
      }
    }

    "return a book by calling the endpoint with the id in the past" in {
      Get("/api/book/2") ~> libraryRoute ~> check {
        ...
        book shouldBe Some(Book(2, "JRR Tolkien", "The Load of the Rings"))
      }
    }
    ...
    "return all books written by the author" in {
      Get("/api/book/author/GRR%20Marting") ~> libraryRoute ~> check {
        status shouldBe StatusCodes.OK
        entityAs[List[Book]] shouldBe books.filter(_.author == "GRR Marting")
      }
    }
...
object RouteDSLSpec extends BookJsonProtocol  with SprayJsonSupport {
  ...
  var books = List(
    ...
    Book(2, "JRR Tolkien", "The Load of the Rings"),
  ...
  /*
    ...
    GET /api/book/{id} - return a single book with id
    GET /api/book?id={id} - same
